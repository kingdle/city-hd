<div class="col-md-12 text-center porctlist">
    <div class="conlistpage">
        <ul class="conlist1">
            <li>
                <a href="{{ admin_base_path('auth/top-news') }}">
                    <i class="fa fa-newspaper-o fa-5x"></i><br><span>统计头条</span>
                </a>
            </li>
            <li>
                <a href="{{ admin_base_path('/auth/economic-monitor') }}">
                    <i class="fa fa-line-chart fa-5x"></i><br><span>新区发展监测</span>
                </a>
            </li>
            <li>
                <a href="{{ admin_base_path('/auth/616') }}">
                    <i class="fa fa-stop-circle-o fa-5x"></i><br><span>新区产业监测</span>
                </a>
            </li>
            <li>
                <a href="{{ admin_base_path('/auth/benchmark-area') }}">
                    <i class="fa fa-area-chart fa-5x"></i><br><span>经济发展对标</span>
                </a>
            </li>
            <li>
                <a href="{{ admin_base_path('/auth/area-development') }}">
                    <i class="fa fa-external-link-square fa-5x"></i><br><span>区域发展监测</span>
                </a>
            </li>
            <li>
                <a href="{{ admin_base_path('/project') }}">
                    <i class="fa fa-star fa-5x"></i><br><span>重点项目监测</span>
                </a>
            </li>
            <li>
                <a href="{{ admin_base_path('/auth/search') }}">
                    <i class="fa fa-search fa-5x"></i><br><span>数据检索</span>
                </a>
            </li>
            <li>
                <a href="{{ admin_base_path('/media') }}">
                    <i class="fa fa-download fa-5x"></i><br><span>数字库</span>
                </a>
            </li>
        </ul>
    </div>
</div>
